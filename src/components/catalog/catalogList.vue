    ...
    >
      ...
      >
        <div class="catalog-list__title font-semibold text-lg">
          <RouterLink class="no-underline text-gray-900 hover:text-primary" :to="`/catalog/${cat.id}`">{{ cat.name }}</RouterLink>
        </div>
...
const firstLeveArr = catalogStore.getProductCatalog.filter(el => el.parent_id == 0).map(fel => {
    return {...fel, child:[]}
})
